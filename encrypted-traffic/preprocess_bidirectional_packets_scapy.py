...
for _, row in filtered_servers.iterrows():
    ...
    if matched_label:
        if matched_label not in server_mapping:
            server_mapping[matched_label] = {"Flows": set()}
        # Store flows as tuples of sorted IP pairs to ensure bidirectionality
        server_mapping[matched_label]["Flows"].add(
            tuple(sorted([(row["Source IP"], row["Destination IP"])]))
        )


# Function to create a flow key for bidirectional flows
def create_bidirectional_flow_key(packet):
    ...
    if pcap_file.endswith(".pcap"):
        ...
        for packet in packets:
            if packet.haslayer(IP):
                flow_key = create_bidirectional_flow_key(packet)
                if flow_key:
        ...
        for flow_key, flow_packets in flows.items():
            ...
            if flow_lengths:
                # Determine label based on bidirectional flow matching
                label = None
                for mapping_label, mapping_ips in server_mapping.items():
                    if flow_key in mapping_ips["Flows"]:
                        label = mapping_label
                        break
                ...
                if label:
                    ...
                    data.append(
                        {
                            "Bidirectional_Flow": f"{flow_key[0][0]} <-> {flow_key[0][1]}",
...
output_file = "bidirectional_tls_flows_features.csv"
